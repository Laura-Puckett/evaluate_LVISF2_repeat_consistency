...
library(sp); library(sf); library(nngeo); library(dplyr); library(corrgram); library(raster); library(dplyr)
source("/home/lkp58/LVIS/LVIS_functions.R")
...
# input_filepath = '/scratch/lkp58/LVIS/accuracy_assessment/inputs.csv'
# input_filepath = '//nau.froot.nau.edu/cirrus/scratch/lkp58/LVIS/accuracy_assessment/inputs.csv'
inputs = read.csv(input_filepath, header=T)

input_dir1 = as.character(inputs$input_dir1[num])
input_dir2 = as.character(inputs$input_dir2[num])

filename1 = as.character(inputs$filename1[num])
filename2 = as.character(inputs$filename2[num])

LVIS_Inst_1 = as.character(inputs$LVIS_Inst_1[num])
LVIS_Inst_2 = as.character(inputs$LVIS_Inst_2[num])
...
arrayLength = 20
out_filename = as.character(inputs$outname[num])
...
## read in data
pts1_orig = st_as_sf(LVIS_TXT_to_points(paste0(input_dir1, filename1), LVIS_Inst_1))
pts2_orig = st_as_sf(LVIS_TXT_to_points(paste0(input_dir2, filename2), LVIS_Inst_2))
...
print(paste0('overlap extent', old_extent))
...
for(arrayNum in 1:(arrayLength-1)){
  ...
  if(nrow(pts1) >0 && nrow(pts2) > 0){
    ...
    if(length(unlist(join_result$dist))==0){
...
# pdf(paste0(outDir, out_filename, '.pdf'))
# 
